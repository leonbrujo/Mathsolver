  ...
  if (!user) return (
    <div style={{ background: '#0D0D1A', minHeight: '100dvh', color: 'white', fontFamily: 'var(--font-sans)', position: 'relative', overflow: 'hidden' }}>
      <style>{`
        ...
        .glow1 { position:absolute;width:500px;height:500px;background:radial-gradient(circle,rgba(124,58,237,0.22) 0%,transparent 70%);top:-150px;left:-150px;pointer-events:none; }
        .glow2 { position:absolute;width:400px;height:400px;background:radial-gradient(circle,rgba(6,182,212,0.18) 0%,transparent 70%);top:50px;right:-100px;pointer-events:none; }
        .glow3 { position:absolute;width:350px;height:350px;background:radial-gradient(circle,rgba(236,72,153,0.12) 0%,transparent 70%);bottom:-50px;left:35%;pointer-events:none; }
        .pill { display:inline-flex;align-items:center;gap:6px;padding:6px 14px;border-radius:20px;font-size:12px;border:1px solid; }
        .clerk-override .cl-rootBox { width:100% }
        .clerk-override .cl-card { background:transparent!important;border:none!important;box-shadow:none!important;padding:0!important; }
        .clerk-override .cl-headerTitle,.clerk-override .cl-headerSubtitle { display:none!important; }
      `}</style>
      <div className="glow1" /><div className="glow2" /><div className="glow3" />

      <div style={{ position: 'relative', zIndex: 1 }}>
        {/* Nav */}
        <nav style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 32px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <div style={{ width:40, height:40, background:'linear-gradient(135deg,#7C3AED,#EC4899)', borderRadius:12, display:'flex', alignItems:'center', justifyContent:'center', fontSize:20, fontWeight:500 }}>∑</div>
            <div>
              <div style={{ fontSize:17, fontWeight:500 }}>MathSolver AI</div>
              <div style={{ fontSize:11, color:'rgba(255,255,255,0.4)', marginTop:-2 }}>by Quiz Quests</div>
            </div>
          </div>
          <div style={{ background:'linear-gradient(135deg,rgba(124,58,237,0.3),rgba(6,182,212,0.3))', border:'1px solid rgba(124,58,237,0.4)', borderRadius:20, padding:'5px 14px', fontSize:12, color:'#C4B5FD' }}>
            ✨ Free to start
          </div>
        </nav>

        {/* Hero */}
        <div style={{ display:'grid', gridTemplateColumns:'1fr 320px', gap:40, padding:'36px 32px 20px', alignItems:'start', maxWidth:1100, margin:'0 auto' }}>
          <div>
            <div style={{ display:'inline-flex', alignItems:'center', gap:6, background:'rgba(236,72,153,0.15)', border:'1px solid rgba(236,72,153,0.35)', borderRadius:20, padding:'5px 14px', fontSize:12, color:'#F9A8D4', marginBottom:16 }}>
              <div style={{ width:6, height:6, background:'#EC4899', borderRadius:'50%', animation:'pulse 1.5s infinite' }} />
              AI-powered math tutor
            </div>
            <h1 style={{ fontSize:40, fontWeight:500, lineHeight:1.15, marginBottom:14 }}>
              Stuck on math?<br />
            ...
            <p style={{ fontSize:15, color:'rgba(255,255,255,0.55)', lineHeight:1.7, marginBottom:24, maxWidth:480 }}>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:28 }}>
              {[
                { label:'Algebra', color:'rgba(124,58,237,0.12)', border:'rgba(124,58,237,0.35)', text:'#C4B5FD' },
                { label:'Calculus', color:'rgba(6,182,212,0.12)', border:'rgba(6,182,212,0.35)', text:'#67E8F9' },
                { label:'Geometry', color:'rgba(52,211,153,0.12)', border:'rgba(52,211,153,0.35)', text:'#6EE7B7' },
                { label:'Arithmetic', color:'rgba(236,72,153,0.12)', border:'rgba(236,72,153,0.35)', text:'#F9A8D4' },
                { label:'Statistics', color:'rgba(251,191,36,0.12)', border:'rgba(251,191,36,0.35)', text:'#FCD34D' },
              ].map(p => (
                <div key={p.label} className="pill" style={{ background:p.color, borderColor:p.border, color:p.text }}>{p.label}</div>
              ))}
            </div>
            <div style={{ display:'flex', alignItems:'center', gap:12, background:'rgba(52,211,153,0.08)', border:'1px solid rgba(52,211,153,0.25)', borderRadius:12, padding:'12px 16px' }}>
              <div style={{ fontSize:22 }}>🎁</div>
              <div style={{ fontSize:13, color:'rgba(255,255,255,0.7)' }}>
                <span style={{ color:'#34D399', fontWeight:500 }}>3 free solves</span> when you sign up — no credit card. Then <span style={{ color:'#34D399', fontWeight:500 }}>$3.99/month</span> for unlimited.
              </div>
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginTop:28 }}>
              {[
                { icon:'📐', title:'Step-by-step', desc:'Every problem broken into clear steps with full explanations.' },
                { icon:'📷', title:'Snap a photo', desc:'Point your camera at any problem — textbook or handwritten.' },
                { icon:'💡', title:'Tips to learn', desc:'Each solution ends with a tip so you actually understand it.' },
              ].map(f => (
                <div key={f.title} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:14, padding:16 }}>
          ...
          {/* Login card */}
          <div style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:20, padding:24, backdropFilter:'blur(10px)' }}>
            <div style={{ fontSize:16, fontWeight:500, textAlign:'center', marginBottom:4 }}>Start solving now</div>
            <div style={{ fontSize:12, color:'rgba(255,255,255,0.4)', textAlign:'center', marginBottom:20 }}>Join thousands of students</div>
              ...
              <SignIn
                appearance={{
                  elements: {
                    ...
                    socialButtonsBlockButton: { background: 'white', color: '#333', borderRadius: 10, fontSize: 13 },
                    dividerLine: { background: 'rgba(255,255,255,0.1)' },
                    dividerText: { color: 'rgba(255,255,255,0.35)', fontSize: 11 },
                    formFieldInput: { background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: 'white', borderRadius: 10, fontSize: 13 },
                    formFieldLabel: { color: 'rgba(255,255,255,0.6)', fontSize: 12 },
                    formButtonPrimary: { background: 'linear-gradient(135deg,#7C3AED,#06B6D4)', borderRadius: 10, fontSize: 14, fontWeight: '500' },
                    footerActionText: { color: 'rgba(255,255,255,0.4)', fontSize: 11 },
                    footerActionLink: { color: '#A78BFA', fontSize: 11 },
