            ...
            <div style={{flexShrink:0,
              ...
              <div style={{perspective:"320px",width:160,height:160,
                ...
                <div key={scanFace} style={{
                  ...
                  {([
                    {fc:"F", tr:"translateZ(45px)"},
                    {fc:"B", tr:"rotateY(180deg) translateZ(45px)"},
                    {fc:"R", tr:"rotateY(90deg) translateZ(45px)"},
                    {fc:"L", tr:"rotateY(-90deg) translateZ(45px)"},
                    {fc:"U", tr:"rotateX(90deg) translateZ(45px)"},
                    {fc:"D", tr:"rotateX(-90deg) translateZ(45px)"},
                  ] as {fc:string,tr:string}[]).map(({fc,tr})=>{
                    const ck = centerMap[fc] ?? "X";
                    const faceHex = HEX[ck] ?? "#1a1a2e";
                    const isTarget = fc === scanFace;
                    const isScanned = !cubeState[fc]?.includes("X");
                    const faceStr = cubeState[fc] ?? "XXXXXXXXX";
                    return (
                      <div key={fc} style={{
                        position:"absolute",width:90,height:90,transform:tr,
                        background: isScanned||isTarget ? faceHex : "#141428",
                        border:`${isTarget?3:1.5}px solid ${
                          isTarget?"#fff":isScanned?"rgba(255,255,255,0.35)":"rgba(255,255,255,0.08)"}`,
                        borderRadius:6,
                        boxShadow:isTarget?`0 0 28px ${faceHex}99`:"none",
                        overflow:"hidden",
                      }}>
                        {/* Real 3x3 sticker grid */}
                        <div style={{position:"absolute",inset:0,
                          display:"grid",gridTemplateColumns:"repeat(3,1fr)",
                          gridTemplateRows:"repeat(3,1fr)",gap:2,padding:3}}>
                          {Array(9).fill(0).map((_,i)=>{
                            const c = faceStr[i] ?? "X";
                            const cellHex = (c==="X"||!isScanned&&!isTarget)
                              ? "transparent"
                              : (HEX[c] ?? faceHex);
                            return (
                              <div key={i} style={{
                                background:cellHex,
                                borderRadius:2,
                                border: cellHex==="transparent"?"none":"1px solid rgba(0,0,0,0.2)",
                                boxShadow: cellHex!=="transparent"?"inset 0 1px 2px rgba(255,255,255,0.2)":"none",
                              }}/>
                            );
                          })}
                        </div>
                        ...
                        {isScanned && !isTarget && (
                          <div style={{position:"absolute",top:3,right:3,
                            width:16,height:16,borderRadius:"50%",background:"#4CAF50",
                            display:"flex",alignItems:"center",justifyContent:"center",
                            fontSize:9,color:"#fff",fontWeight:900,zIndex:2}}>✓</div>
                        )}
                      </div>
                    );
