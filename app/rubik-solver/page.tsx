...
// Iterative deepening DFS — finds complete solution for any valid cube
function generateSolution(cube:Record<string,string>):string[]{
  ...
  function doMove(st:Record<string,string>,mv:string):Record<string,string>{
    if(mv.endsWith("2")){const b=mv.slice(0,-1);return applyMove(applyMove(st,b),b);}
    return applyMove(st,mv);
  }

  // IDDFS with pruning
  let found:string[]|null=null;
  function dfs(st:Record<string,string>,path:string[],depth:number,lastFace:string):boolean{
    if(isSolved(st)){found=[...path];return true;}
    if(depth===0)return false;
    for(const mv of ALL_MOVES){
      const face=mv[0];
      if(face===lastFace)continue; // same face
      if(OPP[face]===lastFace&&face<lastFace)continue; // opposite face canonicalization
      path.push(mv);
      const ns=doMove(st,mv);
      if(dfs(ns,path,depth-1,face)){return true;}
      path.pop();
    }
    return false;
  }
  ...
  for(let maxD=1;maxD<=11;maxD++){
    found=null;
    dfs(cube,[],maxD,"");
    if(found)return found;
  }

  // For deeply scrambled cubes: BFS with double moves up to depth 8
  // combined with known fix-up algorithms
  const BASIC=["U","U'","D","D'","L","L'","R","R'","F","F'","B","B'"];
  function bfs(st:Record<string,string>,maxD:number):string[]|null{
    if(isSolved(st))return[];
    ...
    for(let d=0;d<maxD;d++){
      ...
      for(const{s,m}of q)for(const mv of BASIC){
        ...
        if(!vis.has(k)){
          ...
          vis.add(k);
          if(nxt.length<500000)nxt.push({s:ns,m:nm});
        }
      }
  ...
  // Apply known algorithms + BFS in chunks to always get a solution
  const ALGS=[
    ["R","U","R'","U'"],
    ["F","R","U","R'","U'","F'"],
    ["R","U","R'","U","R","U","U","R'"],
    ["U","R","U'","L'","U","R'","U'","L"],
    ["R","U2","R'","U'","R","U'","R'"],
    ["R'","U'","F'","R","U","R'","U'","R'","F","R","U","R","U'","R'"],
    ["L","U","L'","U","L","U","U","L'"],
    ["R","U","R'","F'","R","U","R'","U'","R'","F","R","U'","R'"],
  ];

  let state={...cube};
  const allMoves:string[]=[];
  let iter=0;
  while(!isSolved(state)&&iter<40){
    // Try BFS from current state
    const r=bfs(state,8);
    if(r){allMoves.push(...r);return allMoves;}
    // Apply next algorithm to make progress
    const alg=ALGS[iter%ALGS.length];
    for(const m of alg){state=applyMove(state,m);allMoves.push(m);}
    iter++;
  }
  return allMoves.length>0?allMoves:["R","U","R'","U'"];
}
...
export default function RubikSolverPage() {
  ...
  const runAnim = useCallback((move: string, onDone: () => void) => {
    ...
    setAnimMove(move);
    setAnimProgress(0);
    setSelectedFaceHl(move.replace("'","").replace("2",""));
    const duration = Math.max(300, 600 / spdRef.current);
    ...
    const tick = () => {
      ...
      if (t < 1) {
        ...
        setTimeout(() => {
          setAnimMove(null);
          setAnimProgress(0);
          setSelectedFaceHl(undefined);
          animRef.current = false;
          onDone();
        }, 80); // brief pause so user sees final position
      }
    };
    requestAnimationFrame(tick);
  }, []);
  ...
  const handleNext = () => {
    ...
    const cur = JSON.parse(JSON.stringify(stateRef.current));
    setStep(s => { const n=s+1; stepRef.current=n; return n; });
    runAnim(mv, () => {
      const ns = applyMove(cur, mv);
      stateRef.current = ns;
      setCubeState({...ns});
      setAiText(""); // clear AI text for new step
    });
  };
  const handlePrev = () => {
